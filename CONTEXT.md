...
**Toolchain**:
The compiler family that builds a Plugin: `msvc` or `mingw` for C/C++, `fpc` (Free Pascal) for Pascal, `rust` (Cargo with MSVC targets) for Rust.
...
**Plugin API**:
NSIS's `pluginapi.c` and headers, or `nsis.pas` for Pascal, which give a Plugin access to the NSIS stack and variables. For `msvc`, `mingw` and `fpc` it is compiled into the Plugin from the NSIS source of a given version; a Rust Plugin is supplied none and brings its own.
_Avoid_: SDK, ExDLL, pluginapi.lib

**Sources and Project**:
The two ways a Plugin points at its code. `msvc` and `mingw` take **sources**, globs matching every file to compile. `fpc` and `rust` take a **project**, the single file their Toolchain reads to find the rest: a `.dpr`, `.lpr` or `.pas`, or a `Cargo.toml`. A Toolchain takes exactly one of them.
